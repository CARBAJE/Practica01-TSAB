...
def sigmoid_contrast(image, alpha, beta):
    image = image.astype(np.float32)  # Convertir a flotante
...
def fitness_function(params, image):
    ...
    contrast = np.std(enhanced_image)  # Desviación estándar como métrica de contraste
    return -contrast  # Minimizar el negativo del contraste para maximizarlo
...
def optimize_contrast(image):
    ...
    Nc = 2
    Nm = 2
    lb = np.array([0.01, 0])  # Alpha mínimo 0.01, beta en el rango [0,255]
    ub = np.array([1, 255])
...
for run in range(5):
    ...
    # Almacenar resultados de cada ejecución
